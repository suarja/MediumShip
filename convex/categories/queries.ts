...
import type { QueryCtx } from "../_generated/server"; // used by loadTenantCategories helper
...
export const listPublishedCategories = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (configured.length > 0) {
      ...
      return Promise.all(
        sorted.map(async (category) => {
          const count = await contentCategoryCounts.count(ctx, {
            bounds: { prefix: [args.tenantSlug, "published", category.label] },
          });
          return {
    ...
    // No configured categories: discover from aggregate keys.
    // Fall back to a bounded scan of the published corpus for category discovery
    // (rare path — only when no categories are configured in the CMS).
    ...
    for (const content of contents) {
      seen.set(content.category, (seen.get(content.category) ?? 0) + 1);
    }
