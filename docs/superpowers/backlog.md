...
- **Player YouTube contrôlable (IFrame Player API)** → **🔧 En refactor** : 1ère implémentation (`docs/superpowers/plans/2026-06-10-mobile-slice-youtube-iframe-player.md`) a greffé YouTube dans le `PersistentMediaPlayer` natif → bugs émergents (autoplay selon le chemin de nav, contrôles désynchronisés, complexité). **Refactor décidé** : `docs/superpowers/plans/2026-06-10-refactor-youtube-self-contained-player.md` — player **inline auto-contenu** sur `/player`, une seule source de vérité, suppression bridge/host/slot/engine/`youtubePlayIntent`. **Pas de mini-player cross-nav YouTube** (background/PiP impossibles derrière WebView — limite assumée). Reprise/historique conservés. Fix Convex `author` (feed validator) commité au passage. Suivi séparé : « back recharge le feed ». Aujourd'hui la lecture YouTube est un `<iframe>` **nu** (boîte noire : pas de position, recharge à zéro au remount). Cible : `react-native-youtube-iframe` → capter la position, **reprendre** (`seekTo`), faire entrer YouTube dans le **player persistant** (3ᵉ adaptateur du seam `PlaybackEngine`, kind de session `youtube`) → reprise/historique/mini-player **gratuits, zéro backend**. **Limites assumées** : pas de PiP OS ni de background YouTube (impossible derrière WebView ; mini-player docké à la place) ; `embeddable=false` reste filtré + repli deep-link. Stratégique car le produit est majoritairement du contenu **importé de YouTube** → type vidéo primaire.
