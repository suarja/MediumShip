...
- **Slice v1 — Premium gratuit + review prompts + flag CMS insights (EN COURS, pour la 1ʳᵉ soumission).** Branche depuis la branche Slice 4 (merge groupé dans `dev`). (A) `PAYMENTS_ENABLED=false` masque RevenueCat ; **« Essayer Premium gratuitement »** → `startFreePremium` (source `trial`, permanent) → pro. (B) `premiumInsights` toggle CMS effectif (off ⇒ retiré UI + cron). (C) `expo-store-review` once-only sur 3 triggers (passage Premium, ajout liste, 1er bookmark ; calque `../editia/mobile/lib/services/reviewService.ts`). Plan : `docs/superpowers/plans/2026-06-11-monet-slice-v1-free-premium-review.md`.
