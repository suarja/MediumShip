...
- **Perf Slice A — Feed : pool de candidats borné et ciblé.** **✅ Livré (mergé `dev` 2026-06-11).** `getDiscoveryFeed` ne lit plus tout le corpus : `loadFeedCandidates` = récence (index `by_tenant_and_status_and_publishedAt`) + slices par catégorie d'affinité/intérêt + échantillon aléatoire profond. **Vérifié orchestrateur** : scoring.ts 0 diff (perso identique), 446 tests Convex PASS, tsc clean, simulation MCP = **327 docs / 0,387 MB vs 13,97 MB (~36×)**, plus de warning « Many bytes read ». Plan : `docs/superpowers/plans/2026-06-11-perf-slice-a-feed-bounded-candidates.md`.
  - **⚙️ À déployer** : le nouvel index `by_tenant_and_status_and_publishedAt` se construit au prochain `convex dev`/deploy (build d'index sur ~11k lignes).
  - **Micro-suivi parqué (data-consistency)** : `contents.category` est stocké de façon **incohérente** — la masse (Wikipedia : science/philosophy/analyse/podcast) est déjà normalisée (lowercase), mais ~7 items CMS sont capitalisés (`Analyse`, `Podcast`…). Le fetch ciblé par catégorie (clé normalisée) rate ces 7 (0,06 %, retombent dans la récence). Fix propre = normaliser `category` à l'écriture (ingest + CMS) + backfill, ou ajouter un champ `categoryKey` normalisé indexé. Non bloquant.
- **Perf Slice B — Counts catégories via `@convex-dev/aggregate`.** **✅ Livré (mergé `dev` 2026-06-11).** `listPublishedCategories` lit les counts en O(log n) via le composant (préfixe `[tenantSlug, "published", label]`) ; `.collect()` de `contents` conservé seulement en **fallback** (tenant sans catégorie configurée — non atteint par demo-media, 21 catégories). **Vérifié orchestrateur** (grep indépendant) : tous les write-paths `contents` câblés (ingest, cms create/update/setStatus, seed) ; replace lit l'ancien doc avant mutation ; patch `articleBody` (immersion) skip justifié ; aucun delete `contents`. Backfill + cron de réconciliation quotidien (anti-drift). 452 tests Convex PASS, tsc clean, comportement préservé. Plan : `docs/superpowers/plans/2026-06-11-perf-slice-b-category-counts-aggregate.md`.
...
> **⚠️ Révision 2026-06-11 — v1 SANS paiement réel.** Le store v1 ne câble pas l'IAP : **gating premium réel** mais on devient pro via **« Essayer Premium gratuitement »** (zéro prix, zéro RevenueCat → review Apple simple). **Slice 4 (RevenueCat) est codée + testée mais MERGÉE DÉSACTIVÉE derrière `PAYMENTS_ENABLED=false`** ; ré-activer le paiement plus tard = flipper 1 flag. Nouvelle slice : `docs/superpowers/plans/2026-06-11-monet-slice-v1-free-premium-review.md`.
...
- **Slice 3 — Sweep badges natifs.** **✅ Livré (mergé `dev`).** Badge d'accès affiché **uniquement si bloqué pour cet utilisateur** (`resolveAccessBadge` sur `requiresPremium`/`requiresSignIn`).
- **Slice 4 — RevenueCat + IAP.** **✅ Code complet + testé en sandbox (Test Store), branche `feat/monet-slice-4-revenuecat-iap` — sera mergée DÉSACTIVÉE par `PAYMENTS_ENABLED` via la slice v1.** Composant `convex-revenuecat` + webhook fail-closed + sync (`isPro` dérivé de `hasEntitlement`, annulation≠révocation), `react-native-purchases`, paywall réel + restore + manage-subscription, gating réel (`PREMIUM_PAYMENT_DEFERRED=false`), entitlement id configurable par env. Plan : `docs/superpowers/plans/2026-06-10-monet-slice-4-revenuecat-iap.md`. **En suspens** (réactivation = `PAYMENTS_ENABLED=true`).
- **Slice v1 — Premium gratuit + review prompts + flag CMS insights.** **✅ Livré (mergé `dev` 2026-06-11, smoke device + CMS toggle OK).** (A) `PAYMENTS_ENABLED=false` masque RevenueCat ; **« Essayer Premium gratuitement »** → `startFreePremium` (source `trial`, permanent) → pro. (B) `premiumInsights` toggle CMS effectif (off ⇒ retiré UI + cron ; label CMS aligné « Ta lecture du jour »). (C) `expo-store-review` once-only sur 3 triggers (passage Premium, ajout liste, 1er bookmark). + polish paywall, fix flash boot feed (`isConnected===false`). Plan : `docs/superpowers/plans/2026-06-11-monet-slice-v1-free-premium-review.md`.
- **Différé — Réintroduction du paiement** : `PAYMENTS_ENABLED=true` (Slice 4 se ré-active) + setup App Store Connect/RevenueCat + migration users `trial` → payants.
