...
- [ ] **Step 5 (Jest) :** **ligne « Abonnement » du membre** (`src/components/profile/profile-library-rows.tsx`, branche `isMember`) : si `PAYMENTS_ENABLED` → `openManageSubscriptions()` (existant) ; si **`!PAYMENTS_ENABLED`** → ouvrir une **bottom sheet de remerciement** (NEW `membership-thanks-sheet.tsx`) : titre « Merci pour le soutien », corps « Tu bénéficies de… » + liste des avantages premium actifs (Ta lecture du jour, offline, listes illimitées, salon membres). Pas de mention d'abonnement/prix. Adapter le libellé de la ligne (`rows.subscription`) en conséquence (« Ton accès Premium » plutôt que « Gérer/résilier »). Tester : flag off → sheet remerciement, flag on → manage natif.
- [ ] **Step 6 :** i18n `premium`/`paywall` (`freeTrialCta`, `freeTrialNote` « Sans engagement », `thanks.title`/`thanks.body`/`thanks.benefits`…), fr+en. `npm test` + `npm run test:convex` + tsc clean. **Commit** — `feat(monet): free premium activation + membership thanks sheet behind PAYMENTS_ENABLED`.
